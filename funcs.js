...
function criaTabelaGraficos(ganttContent, mes) {
    ...
    tdBar2.innerHTML = 'ALOCAÇÃO SEMANAL<br><span style="font-size: 13px;">(Interventores por Dia)</span><span style="font-size: 16px;"> <br></span>';
    ...
    // === Loop dos técnicos ===
 	dados.tecnicos.forEach((tecnico, idxTec) => {
    	...
    	while (i < tranchasMes.length) {
    	    ...
    	    // Verifica se há sobreposição (mais de uma tarefa no mesmo dia)
			const sobreposicao = tarefasSpan.length > 1;
			divBarra.title = tarefasSpan.map(tt => {
    			return sobreposicao
